...
# 11. 기획안 확인 절차 (코드 리뷰 / 기능 구현 공통)

이 프로젝트는 `planning/specs/` 하위에 기능별 기획 문서를 관리하고,
`planning/specs/feature.toml` 파일이 "기능명 → 기획 문서 경로"를 매핑하는
인덱스(레지스트리) 역할을 한다.

사용자가 특정 기능(예: "토스 페이먼츠", "회원", "쿠폰", "재고" 등)과 관련된
코드 리뷰, 구현, 수정을 요청하면 다음 절차를 반드시 따를 것:

1. `planning/specs/feature.toml`을 먼저 읽어서, 요청된 기능명과 매칭되는
   entry(기획 문서 경로)를 찾는다.
2. 매칭되는 entry가 있으면 해당 경로의 기획 문서(.md)를 읽고 요구사항, 정책,
   예외 케이스를 파악한다.
3. 대상 코드(컨트롤러/서비스/도메인 로직 등)가 기획 문서의 요구사항과
   일치하는지 항목 단위로 검토한다.
4. 기획안과 다르게 구현됐거나 누락된 부분이 있으면, 반드시 근거
   (기획서의 어느 항목과 다른지, 어떤 코드가 문제인지)를 명시하며 지적한다.
   추측으로 "아마 이런 의도였을 것"이라고 판단하지 않는다.
5. `feature.toml`에 요청된 기능명과 매칭되는 entry가 없으면, 임의로
   판단해서 넘어가지 말고 사용자에게 "feature.toml에서 해당 기능을 찾지
   못했다"고 알리고 어떤 기획 문서를 참고해야 하는지 되묻는다.
6. 기획 문서 자체가 모호하거나 구현과 상충되는 경우, 코드가 틀렸다고
   단정하지 말고 어느 쪽이 최신 결정인지 사용자에게 확인을 요청한다.

이 절차는 "코드 리뷰해줘", "기획대로 잘 됐는지 봐줘", "이 기능 구현해줘" 등
기능명이 언급된 모든 작업 요청에 적용된다.
